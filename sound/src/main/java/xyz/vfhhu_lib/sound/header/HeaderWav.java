package xyz.vfhhu_lib.sound.header;
...
public class HeaderWav {
    ...
    public HeaderWav(InputStream mFStream)
    {
        byte[] mBuffer = null;
        mBuffer = new byte[4];
// Read RIFF Symbol
        try {
            ...
            mFStream.read(FORMAT);// 格式类型 1:PCM
...
//        header[32] = (byte) (2 * 16 / 8); // block align
    ...
    public int getLength(){
        return byteToInt(LENGTH3);
    }
    public int getChannel(){
        return byteToInt2(CHANNEL);
    }
    public int getFormat(){
        return byteToInt2(FORMAT);
    }
    public int getSampleRate(){
        return byteToInt(SAMPLE_RATE);
    }
    public int getDeep(){
        //Log.d("DEEP","DEEP"+DEEP[0]+","+DEEP[1]);
        return byteToInt2(DEEP);
    }

    public int getPlayTime(){
        int AvgBytesPerSec = byteToInt2(CHANNEL) * byteToInt(SAMPLE_RATE) * byteToInt2(DEEP) / 8;
        int leng3=byteToInt(LENGTH3);
        int ret=leng3/AvgBytesPerSec;
        if(ret==0 && leng3>0){
            ret=1;
        }
        return ret;
